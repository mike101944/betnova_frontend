<template>
  ...
  // Sports Bets - Now receives selections from GameList
  const sportsBets = ref([])
  
  // Virtuals Bets
  const virtualsBets = ref([])
  
  // Method to add bet from GameList
  const addToBetslip = (selection) => {
    // Check if already exists
    const exists = sportsBets.value.some(bet => bet.id === selection.id)
    if (!exists) {
      sportsBets.value.push(selection)
    }
  }
  
  // Method to remove bet
  const removeFromBetslip = (selectionId) => {
    const index = sportsBets.value.findIndex(bet => bet.id === selectionId)
    if (index !== -1) {
