<template>
          ...
          <div class="flex justify-between items-start">
            <div>
              <div class="flex items-center gap-2 mb-2">
                ...
                <div class="bg-amber-400 text-gray-900 rounded-full px-3 py-1">
                  <span class="text-sm font-bold">{{ bet?.result?.toUpperCase() }}</span>
                </div>
              </div>
              <h1 class="text-2xl font-bold">Bet Details</h1>
            </div>
          ...
          <div class="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-4xl font-black">TSh {{ Number(bet.potentialReturn).toLocaleString() }}</p>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="bg-gray-50 rounded-xl p-4">
                ...
                <p class="text-2xl font-bold text-gray-900">TSh {{ Number(bet.stake).toLocaleString() }}</p>
              </div>
              
              <div class="bg-gray-50 rounded-xl p-4">
                <p class="text-sm text-gray-500 mb-1">Potential Win</p>
                <p class="text-2xl font-bold text-green-600">TSh {{ Number(bet.potentialWin || bet.potentialReturn).toLocaleString() }}</p>
              </div>
              
              <div class="bg-gray-50 rounded-xl p-4">
                <p class="text-sm text-gray-500 mb-1">Tax (12%)</p>
                <p class="text-2xl font-bold text-red-500">-TSh {{ calculateTax(bet).toLocaleString() }}</p>
              </div>
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
              <div class="flex justify-between items-center">
                ...
                <span class="text-2xl font-black text-green-600">TSh {{ calculateFinalPayout(bet).toLocaleString() }}</span>
              </div>
            </div>
          </div>
            ...
            <div class="divide-y divide-gray-200">
              <div v-for="(selection, index) in parsedSelections" :key="index" class="p-6">
                <div class="flex items-start justify-between mb-2">
                  <div>
                    <p class="font-bold text-gray-900">{{ selection.match }}</p>
                    <p class="text-sm text-gray-500 mt-1">{{ selection.league }}</p>
                  </div>
  ...
  const parsedSelections = computed(() => {
    ...
    try {
      ...
      return JSON.parse(bet.value.selections)
    } catch (e) {
  ...
  const fetchBetDetails = async () => {
    ...
    try {
      ...
      if (route.params?.id && route.state?.currentBet) {
        bet.value = route.state.currentBet
      } else {
        // Otherwise fetch from API
        const { fetchBetById } = useBets()
        bet.value = await fetchBetById(route.params.id)
      }
    } catch (err) {
