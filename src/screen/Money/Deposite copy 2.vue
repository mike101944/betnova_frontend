...
const orderId = ref(null) // Add this for tracking HarakaPay order
...
const handleDeposite = async () => {
    ...
    try {
        // Step 1: Initiate payment with HarakaPay
        const paymentResponse = await api.post('/harakapay/pay', {
            phone: userPhone.value,
            amount: Number(amount.value),
            description: 'Account deposit'
        });
        
        console.log('HarakaPay response:', paymentResponse.data);
        
        // Check if payment was initiated successfully
        if (paymentResponse.data.order_id) {
            orderId.value = paymentResponse.data.order_id;
            
            // Step 2: Check payment status (optional - can be done after confirmation)
            const statusResponse = await api.get(`/harakapay/status/${orderId.value}`);
            console.log('Payment status:', statusResponse.data);
            
            // Step 3: If payment successful, update balance in database
            if (statusResponse.data.status === 'successful' || statusResponse.data.status === 'completed') {
                const depositResponse = await api.post('/auth/deposit', { 
                    amount: Number(amount.value) 
                });
                ...
                successMessage.value = 'Payment initiated. Please complete on your phone.';
            } else {
                errorMessage.value = 'Payment failed. Please try again.';
            }
        } else {
            errorMessage.value = paymentResponse.data.message || 'Payment initiation failed';
        }
                    ...
                    <div data-v-c9b60287="">
                        <div data-v-4b7cd401="" data-v-c9b60287="" class="withdraw-form">
                            <form data-v-4b7cd401="" class="form" @submit.prevent="handleDeposite">
                                ...
                                <div v-if="orderId && !successMessage && !errorMessage" class="info-message" data-v-4b7cd401>
                                    Order ID: {{ orderId }}
                                </div>
