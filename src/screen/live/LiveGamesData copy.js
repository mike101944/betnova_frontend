...
class LiveGameManager {
  ...
  isGameLiveNow(game) {
    ...
    const gameEndTime = new Date(gameDateTime.getTime() + (90 * 60 * 1000)) // 90 minutes
  ...
  isCompleted(game) {
    const gameDateTime = this.parseGameDateTime(game.date, game.time)
    if (!gameDateTime) return false
    const gameEndTime = new Date(gameDateTime.getTime() + (90 * 60 * 1000))
    return this.getCurrentEAT() > gameEndTime
  }

  // Calculate current minute of live game
  calculateCurrentMinute(game) {
    ...
    // Add half time break (15 minutes) if elapsed > 45
    if (elapsedMinutes > 45) {
      elapsedMinutes += 15 // Half time break
    }
    
    return Math.min(elapsedMinutes, 90)
  }

  // Get half time status
  getHalfTimeStatus(minute) {
    if (minute >= 45 && minute <= 60) return 'HALF TIME'
    if (minute > 60) return 'SECOND HALF'
    return 'FIRST HALF'
  }
  ...
  // Generate random goal events based on minute and game strength
  generateGoals(game, minute) {
    ...
    const homeGoalProb = 0.02 * homeStrength * (minute / 45)
    const awayGoalProb = 0.02 * awayStrength * (minute / 45)
  ...
  updateLiveGame(game) {
    ...
    if (minute >= 90 && game.status !== 'FINISHED') {
    ...
    return {
      ...
      status: minute >= 45 && minute <= 60 ? 'HALF_TIME' : 'LIVE'
    }
  }
  ...
  startLiveGame(game) {
    ...
    if (!exists) {
      this.liveGames.value.push(game)
    }
        ...
        complete: (results) => {
          const allGames = results.data
            ...
            .map((item, index) => ({
              ...
              homeIcon: this.getTeamIcon(item.homeTeam),
              awayIcon: this.getTeamIcon(item.awayTeam),
              league: item.league || '',
