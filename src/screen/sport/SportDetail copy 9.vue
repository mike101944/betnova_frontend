...
const calculateCorrectScoreOdds = (homeOdds, drawOdds, awayOdds) => {
  ...
  if (isNaN(h) || isNaN(d) || isNaN(a) || h < 1 || d < 1 || a < 1) {
    // Generate random odds if main odds are invalid
  ...
  // Generate odds for each score using Poisson distribution approximation
  const scoreOdds = scores.map(score => {
    if (score === 'Other') {
      // Other scores have higher odds
    ...
    let probability = 0
    const homeFactor = Math.exp(-expectedHomeGoals) * Math.pow(expectedHomeGoals, homeScore) / this.factorial(homeScore)
    const awayFactor = Math.exp(-expectedAwayGoals) * Math.pow(expectedAwayGoals, awayScore) / this.factorial(awayScore)
    
    probability = homeFactor * awayFactor
    ...
    if (homeScore === awayScore && homeScore > 1) {
      odds = odds * 1.3 // High draws are less likely
    }
    if (Math.abs(homeScore - awayScore) > 2) {
      odds = odds * 1.2 // Big wins are less likely
    }
  ...
  // Sort by odds (lowest to highest)
...
const generateDerivedOdds = () => {
  ...
  console.log('Generated Correct Score Odds:', correctScoreOdds.value.slice(0, 5))
}
...
<template>
  <div class="h-full bg-sky-900 text-white p-4">
    ...
    <!-- Warning message - only one selection allowed -->
